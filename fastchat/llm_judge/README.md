...
### Evaluate a local model compared with gpt-3.5 by lmdeploy service
```sh
bash judge_workflow.sh -p [MODEL_PATH] -m [MODEL_NAME] -d [DEVICE_ID] -t [TP_NUM] -b [BACKEND: lmdeploy]
```

e.g.
```sh
bash judge_workflow.sh -p ~/code/llm/lmdeploy/downloads/vicuna-13b-v1.5/ -m vicuna-13b-v1.5-0113 -d 0 -t 1 -b lmdeploy
```
